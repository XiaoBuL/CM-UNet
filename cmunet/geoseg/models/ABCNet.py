...
class ContextPath(nn.Module):
    def __init__(self, pretrained=True, *args, **kwargs):
        ...
        self.resnet = timm.create_model("swsl_resnet18", features_only=True, output_stride=32,
                                  out_indices=(2, 3, 4), pretrained=False) # , pretrained_cfg_overlay=dict(file='~/.cache/huggingface/hub/models–timm–resnet18.a1_in1k/pytorch_model.bin'))        # build decoder layers
        state_dict = torch.load('/data0/mushui/.cache/huggingface/hub/pytorch_model.bin')
        msg = self.resnet.load_state_dict(state_dict, strict=False)
        print('[INFO] ', msg)
