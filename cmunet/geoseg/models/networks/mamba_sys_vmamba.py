...
class VSSMMamba(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2], 
        ...
        #                           out_indices=(1, 2, 3, 4), pretrained=True)        # build decoder layers
        # base_dims = 64
        ...
        if self.final_upsample == "expand_first":
            ...
            self.output = nn.Sequential(ConvBNReLU(self.embed_dim, self.embed_dim),
                                        nn.Dropout2d(p=drop_rate, inplace=True),
                                        Conv(self.embed_dim, num_classes, kernel_size=1))

        if self.training:
            self.conv4 = nn.Sequential(ConvBNReLU(self.embed_dim * 2, self.embed_dim),
                                       nn.Dropout2d(p=drop_rate, inplace=True),
                                       Conv(self.embed_dim, num_classes, kernel_size=1))

            self.conv3 = nn.Sequential(ConvBNReLU(self.embed_dim, self.embed_dim),
                                       nn.Dropout2d(p=drop_rate, inplace=True),
                                       Conv(self.embed_dim, num_classes, kernel_size=1))

            self.conv2 = nn.Sequential(ConvBNReLU(self.embed_dim, self.embed_dim),
                                       nn.Dropout2d(p=drop_rate, inplace=True),
                                       Conv(self.embed_dim, num_classes, kernel_size=1))
        
            # nn.Conv2d(base_dims * 2, num_classes, 1, bias=False)
        ...
        self.AFFs = nn.ModuleList([
            ...
            # AFF(96 * 7, 96 * 2),
            # AFF(96 * 7, 96 * 4),
            MSFFA(base_dims * 7, base_dims * 1),
            MSFFA(base_dims * 7, base_dims * 2),
            MSFFA(base_dims * 7, base_dims * 4),
        ])
        self.apply(self._init_weights)
    ...
    def forward_downfeatures(self, x_downsample):
        ...
        x_down_4 = self.AFFs[2](x_downsample[2], x_down_2_4, x_down_3_4)
