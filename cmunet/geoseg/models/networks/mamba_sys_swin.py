...
class VSSM_SWIN(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2], 
        ...
        if self.final_upsample == "expand_first":
            ...
            self.output = nn.Sequential(ConvBNReLU(embed_dim, embed_dim),
                                        nn.Dropout2d(p=drop_rate, inplace=True),
                                        Conv(embed_dim, num_classes, kernel_size=1))

            
            # nn.Conv2d(in_channels=self.embed_dim,out_channels=self.num_classes,kernel_size=1,bias=False)
